...
const Bookmarks: React.FC<Props> = ({ itemId }) => {
  ...
  }, [itemId, createNew, newBookmark, bookmarkCreateAsync, refetch, refetchBookmarks, bookmarkToggleItemAsync]);
    ...
    <>
      <div className="flex flex-wrap" key={dataUpdatedAt}>
        {map(data?.items, (bookmark) => (
          <div className="w-1/2 p-1 md:w-1/3 lg:w-1/4 xl:w-1/5" key={bookmark.updated}>
