...
const FilterItems: React.FC<FilterItemsProps> = ({ type, defaultGenre, storageKey, onFilter }) => {
  ...
  return (
    <>
      ...
      <Popup visible={isOpen} onClose={handleClose} className="top-0 xl:top-auto">
        ...
        <div className="flex flex-col xl:flex-row xl:justify-between">
          <div className="flex flex-col justify-start w-full xl:w-1/2">
            <Checkbox checked={sortingAsc} onChange={(checked) => setSortingAsc(checked)}>
              Сортировка по возрастанию
            </Checkbox>

            <div className="flex flex-col xl:flex-row xl:justify-between">
              <Checkbox className="w-full xl:w-1/2" checked={skipAds} onChange={(checked) => setSkipAds(checked)}>
                Без рекламы
              </Checkbox>
              <Checkbox className="w-full xl:w-1/2" checked={skipErotic} onChange={(checked) => setSkipErotic(checked)}>
                Без эротики
              </Checkbox>
            </div>
          </div>
