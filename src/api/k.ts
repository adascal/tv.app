...
class KApiClient extends BaseApiClient {
  ...
  constructor(clientId: string = API_CLIENT_ID, clientSecret: string = API_CLIENT_SECRET, baseUrl: string = API_BASE_URL) {
    super(baseUrl);
  ...
  /**
   * Список пользовательских плэйлистов
   * @description Список пользовательских плэйлистов добавленных на портале.
   */
  playlists() {
    return this.get<PlaylistsResponse>(`/v1/playlists`);
  }

  /**
   * Пользовательский плэйлист
   * @description Пользовательский плэйлист добавленный на портале.
   */
  playlist(id: string) {
    return this.get<PlaylistResponse>(`/v1/playlists/${id}`);
  }
